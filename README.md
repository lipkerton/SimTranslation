...
# Принцип работы #
+ Запуск:
    1. Cоздается словарь на основе базового словаря (**базовый словарь - dictionaries/chn_base_dictionary.csv**, **создаваемый словарь - decoded_dictionary.pkl**).
...
+ Процесс перевода:
    ...
    3. Ищет текст в словаре dictionaries/decoded_dictionary.pkl.
    4. Если текст отсутствует в словарe dictionaries/decoded_dictionary.pkl, то выполняется процедура перевода.
...
+ Окончание работы:
    ...
    3. Пользователь имеет возможность открыть текущее состояние словаря dictionaries/decoded_dictionary.pkl (документы dictionaries/dictionary_current_state.csv, dictionaries/dictionary_current_state.txt) и выполнить корректировку словаря прямо там, с последующим сохранением изменений в интерфейсе скрипта с помощью кнопок.
...
+ **'Translate'** - запускает выбранную папку и xprt/xml файлы в ней на перевод.
    + Пока процесс перевода не завершен изменить словарь dictionaries/decoded_dictionary.pkl невозможно.
    + Кнопка 'Translate' будет заблокирована до окончания перевода.
+ **'Quick save changes'** - все переводы в окне вывода будут сохранены в словарь dictionaries/decoded_dictionary.pkl
...
+ **'Open full dictionary'** - будет открыто **текущее состояние** словаря dictionaries/decoded_dictionary.pkl.
...
# Working principle #
+ Startup:
    1. A dictionary is created based on the base dictionary (**base dictionary - dictionaries/chn_base_dictionary.csv**, **created dictionary - decoded_dictionary.pkl**).
...
+ Translation process:
    ...
    3. searches for text in dictionaries/decoded_dictionary.pkl.
...
+ End of work:
    1. When the work on the selected directory is complete, the output window will show all translations that were done exactly by machine translation - this means that they are missing from dictionaries/decoded_dictionary.pkl.
    2. The user can correct translations directly in the output window (in the format <Russian word>;<English translation>;<Chinese translation>) and can immediately save them to the dictionaries/decoded_dictionary.pkl.
...
+ **'Translate'** - starts the selected folder and xprt/xml files in it for translation.
    + Until the translation process is completed, the dictionaries/decoded_dictionary.pkl cannot be changed.
    + The 'Translate' button will be locked until the translation is complete.
+ **'Quick save changes'** - all translations in the output window will be saved to the dictionaries/decoded_dictionary.pkl dictionary.
...
+ **'Open full dictionary'** - the **current state** of dictionaries/dictionaries/decoded_dictionary.pkl will be opened.
